...
# High-confidence: flagged even in example/template files unless the rule has
# an explicit false-positive guard for an obvious placeholder or interpolation.
HIGH_CONFIDENCE = [
    ...
    for file, line_no, content in parse_diff_added_lines(diff_text):
        ...
        for pattern, msg in HIGH_CONFIDENCE:
            if re.search(pattern, content):
                if msg == "private key block" and "${" in content:
                    continue
                if (
                    msg == "connection string with embedded password"
                    and PLACEHOLDER.search(content)
                ):
                    continue
