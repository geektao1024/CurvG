...
test('cycloid requests use an exact provider-independent rolling-circle profile', () => {
  const prompt = '对比摆线与生成它的滚动圆。';
  const artifacts = buildDeterministicCycloidArtifacts(prompt);
  assert.ok(artifacts);
  const spec = composeAnimationSpecFromArtifacts(artifacts);
  const code = compileAnimationSpec(spec);
  ...
  assert.ok(
    spec.objects.some(
      (object) =>
        ...
        object.xExpr === 't-pi-sin(t)' &&
        object.yExpr === '1-cos(t)' &&
        object.domain?.[1] === Math.PI * 2
    )
  );
  assert.match(
    code,
    /ParametricFunction\(lambda t: .*t-np\.pi-np\.sin\(t\).*1-np\.cos\(t\)/
  );
  assert.match(code, /Transform\(obj_rolling_circle/);
  assert.match(code, /Transform\(obj_cycloid_trace/);
  assert.equal(
    deterministicMathReviewForScene(spec, {
      content: JSON.stringify(artifacts.scene),
      provider: 'curvg',
  ...
  assert.equal(
    buildDeterministicCycloidArtifacts('Animate a generic circle.'),
    undefined
  );
});

test('parametric compiler rejects non-allowlisted expressions', () => {
  const artifacts = buildDeterministicCycloidArtifacts(
    'Animate a cycloid generated by a rolling circle.'
  );
  assert.ok(artifacts);
  const spec = composeAnimationSpecFromArtifacts(artifacts);
  const curve = spec.objects.find((object) => object.kind === 'parametric');
  assert.ok(curve);
  curve.xExpr = '__import__(os)';
  assert.throws(
    () => compileAnimationSpec(spec),
    /Expression name is not supported|unsupported characters/
  );
});
